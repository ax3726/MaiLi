...
public abstract class EaseChatRow extends LinearLayout {
    ...
    public EaseChatRow(Context context, EMMessage message, int position, BaseAdapter adapter, boolean bl) {
        super(context);
        this.context = context;
        this.activity = (Activity) context;
        this.message = message;
    ...
    private void setUpBaseView() throws HyphenateException {
        ...
        if (userAvatarView != null) {
            ...
            if (message.direct() == Direct.SEND) {
                ...
                Glide.with(context).load(EaseUI.getInstance().getEaseUser().getAvatar()).diskCacheStrategy(DiskCacheStrategy.ALL)
                        .transform(new GlideCircleTransform(context))
                        .placeholder(R.drawable.ease_default_avatar).into(userAvatarView);
                // EaseUserUtils.setUserAvatar(context, EMClient.getInstance().getCurrentUser(), userAvatarView);
            } else {
                String head_url = message.getStringAttribute("head_url");
                //  String nick_name = message.getStringAttribute("nick_name");
                Glide.with(context).load(head_url).diskCacheStrategy(DiskCacheStrategy.ALL)
                        .transform(new GlideCircleTransform(context))
                        .placeholder(R.drawable.ease_default_avatar).into(userAvatarView);
