...
public class EaseConversationAdapter extends ArrayAdapter<EMConversation> {
    ...
    public EaseConversationAdapter(Context context, int resource,
                                   List<EMConversation> objects, String phone) {
    ...
    public EMConversation getItem(int arg0) {
        if (arg0 < conversationList.size() && arg0 >= 0) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (holder == null) {
            ...
            holder.img_zhidin = (ImageView) convertView.findViewById(R.id.img_zhidin);
        ...
        if (!TextUtils.isEmpty(conversation.getExtField())) {
            holder.img_zhidin.setVisibility(View.VISIBLE);
        } else {
            holder.img_zhidin.setVisibility(View.GONE);
        }
        ...
        if (conversation.getType() == EMConversationType.GroupChat) {
            ...
            String order = "";
            try {
                order = conversation.getLastMessage().getStringAttribute("order");
            } catch (HyphenateException e) {
                e.printStackTrace();
            }

            if (!TextUtils.isEmpty(order)) {
                holder.type_name.setVisibility(View.VISIBLE);
                OrderContent mOrderContent = ParseJsonUtils.getBean((String) order, OrderContent.class);
                if (mOrderContent != null) {
                    holder.type_name.setText(EaseUtils.TypeToOccupation(mOrderContent.getWorkTpye()));
                    if (mOrderContent.getOrderPhone().equals(mCurPhone)) {//是发单人
                        head_url = mOrderContent.getAcceptHeadUrl();
                        nick_name = mOrderContent.getAcceptNickName();

                    } else {
                        head_url = mOrderContent.getSendHeadUrl();
                        nick_name = mOrderContent.getSendNickName();
                    }
                }
            ...
            Glide.with(mContext).load(head_url).diskCacheStrategy(DiskCacheStrategy.ALL)
                    .transform(new GlideCircleTransform(mContext))
                    .placeholder(R.drawable.ease_default_avatar).into(holder.avatar);
            holder.name.setText(TextUtils.isEmpty(nick_name) ? username : nick_name);
        ...
        if (conversation.getUnreadMsgCount() > 0) {
            ...
            holder.unreadLabel.setText(conversation.getUnreadMsgCount() > 99 ? "99" : String.valueOf(conversation.getUnreadMsgCount()));
