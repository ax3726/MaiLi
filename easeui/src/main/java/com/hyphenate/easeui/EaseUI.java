...
public final class EaseUI {
     ...
     * @return
     */
    public synchronized boolean init(Context context, EMOptions options, EaseNotifier.EaseNotificationInfoProvider provider) {
        if (sdkInited) {
            return true;
        }
        appContext = context;
    ...
    void initNotifier(EaseNotifier.EaseNotificationInfoProvider provider) {
        notifier = createNotifier();
        notifier.init(appContext);
        notifier.setNotificationInfoProvider(provider);
    }
                ...
                if (messages != null && messages.size() > 0) {
                    ...
                    for (int i = 0; i < messages.size(); i++) {
                        if (messages.get(i).getType() == EMMessage.Type.TXT) {
                            EMTextMessageBody txtBody = (EMTextMessageBody) messages.get(i).getBody();
                            messages.get(i).addBody(new EMTextMessageBody(EaseUtils.filterMessage(txtBody.getMessage())));
                        }
                    }
                }
    ...
    private EaseUser mEaseUser = null;//用户信息
