...
public class PopupOption {
     ...
     * 点击事件
     */
    private onPopupWindowItemClickListener mItemClickListener;

    /**
     * 一个参数的构造方法，用于弹出无标题的PopupWindow
     */
    public PopupOption(Context context) {
     ...
     * @param title   标题
     */
    public PopupOption(Context context, String title) {
        this.mContext = context;
        this.mTitle = title;
    }
    ...
    public void setItemClickListener(
            onPopupWindowItemClickListener mItemClickListener) {
    ...
    private void addView(View v) {
        LinearLayout lin_layout = (LinearLayout) v
                .findViewById(R.id.layout_popup_add);
        // Title
        ...
        if (mOptions != null && mOptions.length > 0) {
            for (int i = 0; i < mOptions.length; i++) {
                View item = LayoutInflater.from(mContext).inflate(
                        R.layout.lib_popup_base_item, null);
                Button btn_txt = (Button) item
                        .findViewById(R.id.btn_popup_option);
                btn_txt.setText(mOptions[i]);
                if (mColors != null && mColors.length == mOptions.length) {
                    btn_txt.setTextColor(mColors[i]);
                }
                final int finalI = i;
    ...
    public void showPopupWindow() {
        View popupWindow_view = LayoutInflater.from(mContext).inflate(
                R.layout.lib_popup_base_bottom, null);
        // 添加子View
    ...
    public void showCenterPopupWindow() {
        View popupWindow_view = LayoutInflater.from(mContext).inflate(
                R.layout.lib_popup_base_center, null);
        // 添加子View
        addView(popupWindow_view);
        mPopupWindow = new PopupWindow(popupWindow_view,
                ViewGroup.LayoutParams.MATCH_PARENT,
                ViewGroup.LayoutParams.WRAP_CONTENT);
        mPopupWindow.setAnimationStyle(R.style.lib_popwindow_anim_style_center);
        mPopupWindow.setBackgroundDrawable(new BitmapDrawable());
        mPopupWindow.setFocusable(true);
        mPopupWindow.setOutsideTouchable(true);
        mPopupWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
        ...
        if (mPopupWindow != null && !mPopupWindow.isShowing()) {
            mPopupWindow.showAtLocation(popupWindow_view, Gravity.CENTER, 0, 0);
        }
        setWindowAlpa(true);
    }
