...
public class PayHelper {
    ...
    public void WexPay(WEXModel info) {
        if (msgApi == null) {
            msgApi = WXAPIFactory.createWXAPI(MaiLiApplication.appliactionContext, null);
            msgApi.registerApp(Constant.WEXAPPID);// 将该app注册到微信
        }
        PayReq req = new PayReq();
        if (!msgApi.isWXAppInstalled()) {
            ToastUtil.show(MaiLiApplication.appliactionContext, "手机中没有安装微信客户端!");
            return;
        }
        if (info.getData() != null) {
            ...
            req.extData = MaiLiApplication.getInstance().getUserInfo().getPhone();
            msgApi.sendReq(req);
        }
    }
